...
int main(int argc, char *argv[]){
   ...
   if(S_ISDIR(st.st_mode)){//st.mode contiene la informacion del tipo de archivo o directorio y los permisos, y mediante el uso de S_ISDIR comprobamos si es un directorio
      ...
      if (argv[2][longitud-1]=='/'){//En caso de que el directorio acabe en "/"
          //El destino simplemente sera el directorio
	char *mem=malloc(2000);
       snprintf(mem, 2000,"%s%s", argv[2],argv[1]);
        dir_o_arch_pegar=mem;
      }
      ...
      char *espacio_en_memoria=malloc(longitud+20000); //espacio_en_memoria es un puntero que apunta a la memoria dinamica del segundo parametro
         ...
         snprintf(espacio_en_memoria,longitud+20000, "%s/%s", argv[2],argv[1]); //La funcion sprintf es similar a snprintf, solo que esta ultima evita el desbordamiento de buffer por medio del formateo de cadenas de caracteres(string)
...
copiar(argv[1],(const char *)dir_o_arch_pegar);//Hacemos la llamada a funcion para la hacer copia
		 free((char *)dir_o_arch_pegar);//Vaciamos el espacio en memoria reservado para la cadena modificada, ya que esto no se hace automaticamente al acabar el programa, y dejarlo sin liberar podria llava a problemas de fuga de memoria, es decir,
