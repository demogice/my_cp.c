...
int main(int argc, char *argv[]){
   ...
   if(S_ISDIR(st.st_mode)){//st.mode contiene la informacion del tipo de archivo o directorio y los permisos, y mediante el uso de S_ISDIR comprobamos si es un directorio
      ...
      char *espacio_en_memoria=malloc(2000);
      char *memoria=malloc(2000);
...
snprintf(memoria,2000, "%s/", argv[2]);
         snprintf(espacio_en_memoria,2000, "%s%s",memoria, argv[1]); //La funcion sprintf es similar a snprintf, solo que esta ultima evita el desbordamiento de buffer por medio del formateo de cadenas de caracteres(string)
         ...
         free(memoria);//El espacio se asigna dinamicamente, y si no se llega a liberar, con el tiempo el programa ocuparia cada vez mas, llevando a problemas mas serios, como la degradacion de rendimiento, o desgaste de la memoria
      }
   }
